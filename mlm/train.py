...
if args.differential_privacy:
    ...
    if not ((args.lot_size > args.train_batch_size)
        ...
        logger.warning(
            f'Model: {mlm_modelling_dp.args.output_name} - '
        ...
        logger.warning(
            f'DP Model: {mlm_modelling_dp.args.output_name} is training without'
        ...
        mlm_parser.parser.exit()
    try:
        logger.info(f"Training model {mlm_modelling_dp.args.output_name}")
        mlm_modelling_dp.train_model()
        logger.info(f'Model {mlm_modelling_dp.args.output_name} trained succesfully')
    except Exception as ex:
        logger.error(f'Model {mlm_modelling_dp.args.output_name} failed:\n{ex}')
    ...
    try:
        ...
        logger.info(f'Model {mlm_modelling.args.output_name} trained succesfully')
